...
export default function Admin() {
  ...
  const authedFetch = useCallback(async (url: string, options: RequestInit = {}) => {
    ...
    if (!session) { router.push('/login'); throw new Error('no session') }
    return fetch(url, {
      ...options,
      headers: {
  ...
  const loadData = useCallback(async () => {
    try {
      const [sRes, cRes] = await Promise.all([
        authedFetch('/api/admin/students'),
      ...
      // handled via authedFetch redirect
    }
  }, [authedFetch])
  ...
  const toggle = async (id: string, cur: boolean) => {
    const res = await authedFetch('/api/admin/students', { method: 'PATCH', body: JSON.stringify({ id, is_active: !cur }) })
    if (!res.ok) { showToast('حصل خطأ، حاول تاني'); return }
    loadData()
  }
  ...
  const addCourse = async () => {
    if (!nc.title) return
    setLoading(true)
    const res = await authedFetch('/api/admin/courses', { method: 'POST', body: JSON.stringify(nc) })
    setLoading(false)
    if (!res.ok) { showToast('حصل خطأ في إضافة الكورس'); return }
  ...
  const addLesson = async () => {
    ...
    setLoading(true)
    const res = await authedFetch('/api/admin/lessons', { method: 'POST', body: JSON.stringify(nl) })
    setLoading(false)
    if (!res.ok) { showToast('حصل خطأ في إضافة الدرس'); return }
  ...
  return (
    <div style={{minHeight:'100vh', background:'#0a0f1e', fontFamily:"'Segoe UI',sans-serif", direction:'rtl'}}>
      {toast && (
        <div style={{position:'fixed', top:20, left:'50%', transform:'translateX(-50%)', zIndex:200, background:'rgba(255,208,0,0.95)', color:'#0a0f1e', fontWeight:800, fontSize:14, padding:'10px 24px', borderRadius:10, boxShadow:'0 8px 24px rgba(0,0,0,0.3)'}}>{toast}</div>
      )}
          ...
          <div style={{display:'grid', gridTemplateColumns:'1fr 1fr', gap:24}}>
            <div style={{background:'rgba(255,255,255,0.04)', border:'1px solid rgba(255,255,255,0.08)', borderRadius:16, padding:24}}>
              ...
              <div style={{display:'flex', flexDirection:'column', gap:12}}>
                ...
                <button onClick={addCourse} disabled={loading} style={{padding:'12px', borderRadius:10, border:'none', cursor:'pointer', background:'linear-gradient(135deg,#ffd000,#ff8c00)', color:'#0a0f1e', fontWeight:800, fontSize:15, opacity: loading ? 0.7 : 1}}>
            ...
            <div style={{display:'flex', flexDirection:'column', gap:12}}>
              ...
              {courses.length===0 && <p style={{color:'rgba(255,255,255,0.3)', fontSize:14, textAlign:'center', padding:20}}>مفيش كورسات لسه</p>}
            </div>
          </div>
        )}
        ...
        {tab==='lessons' && (
          <div style={{background:'rgba(255,255,255,0.04)', border:'1px solid rgba(255,255,255,0.08)', borderRadius:16, padding:24, maxWidth:600}}>
            ...
            <div style={{display:'flex', flexDirection:'column', gap:12}}>
              ...
              <input type="number" placeholder="رقم الدرس (الترتيب)" value={nl.order_num} onChange={e=>setNl({...nl,order_num:parseInt(e.target.value)||1})} style={inp}/>
              ...
              <button onClick={addLesson} disabled={loading} style={{padding:'12px', borderRadius:10, border:'none', cursor:'pointer', background:'linear-gradient(135deg,#ffd000,#ff8c00)', color:'#0a0f1e', fontWeight:800, fontSize:15, opacity: loading ? 0.7 : 1}}>
